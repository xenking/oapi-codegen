...
func appendPackagePrefix(paramType, prefix string) string {
	if prefix == "" || IsPredeclaredGoIdentifier(paramType) || paramType == "interface{}" {
		return paramType
	}
...
func genResponseUnmarshal(op *OperationDefinition, prefix ...string) string {
	...
	// See: https://github.com/xenking/oapi-codegen/issues/127 for why we handle this in two separate
	// groups.
