...
func doGet(t *testing.T, app *fiber.App, rawURL string) *httptest.ResponseRecorder {
	...
	response := testutil.NewRequest().Get(u.RequestURI()).WithHost(u.Host).WithAcceptJson().GoWithHTTPHandler(t, app)
...
func doPost(t *testing.T, app *fiber.App, rawURL string, jsonBody interface{}) *httptest.ResponseRecorder {
	...
	response := testutil.NewRequest().Post(u.RequestURI()).WithHost(u.Host).WithJsonBody(jsonBody).GoWithHTTPHandler(t, app)
	return response.Recorder
}
...
func TestOapiRequestValidator(t *testing.T) {
	...
	{
		rec := doGet(t, app, "http://not.deepmap.ai/resource")
	...
	{
		rec := doGet(t, app, "http://deepmap.ai/resource")
	...
	{
		rec := doGet(t, app, "http://deepmap.ai/resource?id=500")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		assert.False(t, called, "Handler should not have been called")
		called = false
	}
	...
	{
		rec := doGet(t, app, "http://deepmap.ai/resource?id=foo")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		assert.False(t, called, "Handler should not have been called")
		called = false
	}
	...
	app.Post("/resource", func(ctx *fiber.Ctx) error {
		called = true
	...
	// Send a good request body
	{
		...
		rec := doPost(t, app, "http://deepmap.ai/resource", body)
	...
	app.Get("/protected_resource", func(ctx *fiber.Ctx) error {
		called = true
		return ctx.SendStatus(http.StatusNoContent)
	})
	...
	{
		rec := doGet(t, app, "http://deepmap.ai/protected_resource")
		assert.Equal(t, http.StatusNoContent, rec.Code)
		assert.True(t, called, "Handler should have been called")
		called = false
	}

	app.Get("/protected_resource2", func(ctx *fiber.Ctx) error {
		called = true
		return ctx.SendStatus(http.StatusNoContent)
	})
	// Call a protected function to which we dont have access
	{
		rec := doGet(t, app, "http://deepmap.ai/protected_resource2")
		assert.Equal(t, http.StatusForbidden, rec.Code)
		assert.False(t, called, "Handler should not have been called")
		called = false
	}

	app.Get("/protected_resource_401", func(ctx *fiber.Ctx) error {
		called = true
		return ctx.SendStatus(http.StatusNoContent)
	})
	// Call a protected function without credentials
	{
		rec := doGet(t, app, "http://deepmap.ai/protected_resource_401")
