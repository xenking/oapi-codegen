...
func (w *ServerInterfaceWrapper) FindPets(ctx *fiber.Ctx) error {
	...
	ctx.Request().Header.Set(models.UserScopes, "read,write")
	...
	if err != nil {
		return fiber.NewError(fiber.StatusBadRequest, fmt.Sprintf("Invalid format for parameter tags: %s", err))
	}

	// ------------- Optional query parameter "limit" -------------

	err = runtime.BindQueryParameter("form", true, false, "limit", QueryParams(ctx), &params.Limit)
	if err != nil {
		return fiber.NewError(fiber.StatusBadRequest, fmt.Sprintf("Invalid format for parameter limit: %s", err))
	}
...
func (w *ServerInterfaceWrapper) DeletePet(ctx *fiber.Ctx) error {
	...
	err = runtime.BindStyledParameterWithLocation("simple", false, "id", runtime.ParamLocationPath, ctx.Params("id"), &id)
	if err != nil {
		return fiber.NewError(fiber.StatusBadRequest, fmt.Sprintf("Invalid format for parameter id: %s", err))
	}
