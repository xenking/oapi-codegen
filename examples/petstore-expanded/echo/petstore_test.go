...
func TestPetStore(t *testing.T) {
	...
	result := testutil.NewRequest().Post("/pets").WithJsonBody(newPet).GoWithEchoHandler(t, e)
	...
	result = testutil.NewRequest().Get(fmt.Sprintf("/pets/%d", petId)).WithAcceptJson().GoWithEchoHandler(t, e)
	...
	result = testutil.NewRequest().Get("/pets/27179095781").WithAcceptJson().GoWithEchoHandler(t, e)
	...
	result = testutil.NewRequest().Get("/pets").WithAcceptJson().GoWithEchoHandler(t, e)
	...
	petList = nil
	result = testutil.NewRequest().Get("/pets?tags=TagOfFido").WithAcceptJson().GoWithEchoHandler(t, e)
	assert.Equal(t, http.StatusOK, result.Code())
	err = result.UnmarshalBodyToObject(&petList)
	assert.NoError(t, err, "error getting response", err)
	...
	petList = nil
	result = testutil.NewRequest().Get("/pets?tags=NotExists").WithAcceptJson().GoWithEchoHandler(t, e)
	assert.Equal(t, http.StatusOK, result.Code())
	err = result.UnmarshalBodyToObject(&petList)
	assert.NoError(t, err, "error getting response", err)
	...
	result = testutil.NewRequest().Delete("/pets/7").GoWithEchoHandler(t, e)
	assert.Equal(t, http.StatusNotFound, result.Code())
	err = result.UnmarshalBodyToObject(&petError)
	...
	result = testutil.NewRequest().Delete(fmt.Sprintf("/pets/%d", petId)).GoWithEchoHandler(t, e)
	assert.Equal(t, http.StatusNoContent, result.Code())
	result = testutil.NewRequest().Delete(fmt.Sprintf("/pets/%d", petId2)).GoWithEchoHandler(t, e)
	assert.Equal(t, http.StatusNoContent, result.Code())
